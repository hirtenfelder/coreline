...
int main(void) {
    ...
    while (true) {
        ...
        if (dht22_read(&dht22_data)) {
            ...
            lcd_st7735s_set_line(temp_str, 0);
            lcd_st7735s_set_line(hum_str, 1);
        ...
        if (ublox_neo6m_read_next_nmea_sentence()) {
            ...
            lcd_st7735s_set_line(timestamp, 2);
            lcd_st7735s_set_line(status, 3);
            lcd_st7735s_draw_lines();
            DEV_Delay_ms(1000);
            if (strcmp(status, "A") != 0) {
                lcd_st7735s_set_line("Please wait", 4);
                lcd_st7735s_set_line("GPS init..", 5);
                lcd_st7735s_draw_lines();
                DEV_Delay_ms(1000);
            } else {
                lcd_st7735s_set_line("", 4);
                lcd_st7735s_set_line("", 5);
                lcd_st7735s_draw_lines();
            }
        }
    }
